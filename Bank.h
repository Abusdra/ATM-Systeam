...
#include<fstream>;
using namespace std;
const string ClientFileTxt = "MyFile.txt";
...
namespace Allb {
	...
	bool ChickAccountNumber(string AccountNumber) {
		vector<sclient>Vclient;
		Vclient = LoadClientFromFile(ClientFileTxt);
	...
	void SaveDataToFile(vector<sclient>Vclient) {
		fstream MyFile;
		string Line;
		MyFile.open(ClientFileTxt, ios::out);
		if (MyFile.is_open()) {
			for (sclient c : Vclient) {
	...
	void AddClientToFile(string NewClient) {
		fstream MyFile;
		MyFile.open(ClientFileTxt, ios::out | ios::app);
		if (MyFile.is_open()) {
